...
\item
  ...
  The idea is that if a term is supposed to represent a natural
  transformation in the topos of trees,
  (\mathsf{Set}^{\omega^\mathsf{op}}), then we are entitled to apply
  it to a natural number and observe its behaviour at that stage, but
  the sematics of $\next$ ignore its argument at stage zero, and it is
  this base case that bootstraps the recursion that justifies $\fix$,
  so it seems natural for reduction to also stop there.

  
  
  
  
  %% TODO check if you need more reductions
